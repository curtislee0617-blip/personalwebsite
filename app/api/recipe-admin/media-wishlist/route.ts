...
export async function POST(request: Request) {
  ...
  const recipe = (await getInstagramSavedRecipeCards()).find((entry) => entry.recipeKey === recipeKey);
  ...
  const result = action === "remove"
    ...
    : await supabase.from("recipe_wishlist_entries").upsert({
      ...
      title: recipe.title,
      note: recipe.description || `Saved from ${recipe.sourceLabel ?? "Instagram"}.`,
      href: `/recipes/instagram-saved#recipe-${recipe.slug}`,
      image_url: recipe.thumbnail ?? null,
      cookbook_id: null,
      recipe_id: recipe.recipeKey,
      book_title: "Instagram saved recipes",

  ...
  return (
    <section className="compound-tool">
        ...
        <div className="compound-results">
          ...
          {result ? (
            <>
              ...
              <section className="compound-formation-results" aria-label="Enthalpy and Gibbs energy of formation at 298 kelvin and 1 bar">
                <div><p>Formation properties</p><h3>At 298 K and 1 bar</h3><span>{calculatedCompound.formation ? `${calculatedCompound.formation.phase} · gas phase` : "Not listed in Appendix A.3"}</span></div>
                <article><p><b>ΔH°<sub>f,298</sub></b><span>enthalpy of formation</span></p><strong>{calculatedCompound.formation ? number(calculatedCompound.formation.enthalpy, 6) : "—"}</strong><small>kJ/mol</small></article>
                <article><p><b>ΔG°<sub>f,298</sub></b><span>Gibbs energy of formation</span></p><strong>{calculatedCompound.formation ? number(calculatedCompound.formation.gibbsEnergy, 6) : "—"}</strong><small>kJ/mol</small></article>
              </section>
      ...
      <section className="compound-catalogue">
        ...
        <div className="compound-table-wrap" tabIndex={0} aria-label="Scrollable compound property table"><table><thead><tr><th rowSpan={2}>Compound</th><th rowSpan={2}>Formula</th><th rowSpan={2}>MW<br /><small>g/mol</small></th><th rowSpan={2}>Tᶜ<br /><small>K</small></th><th rowSpan={2}>Pᶜ<br /><small>bar</small></th><th rowSpan={2}>ω</th><th rowSpan={2}>A</th><th rowSpan={2}>B</th><th rowSpan={2}>C</th><th rowSpan={2}>Antoine range<br /><small>K</small></th><th className="compound-formation-heading" colSpan={3}>Enthalpy and Gibbs energy of formation at 298 K and 1 bar</th></tr><tr><th>Phase</th><th>ΔH°<sub>f,298</sub><br /><small>kJ/mol</small></th><th>ΔG°<sub>f,298</sub><br /><small>kJ/mol</small></th></tr></thead><tbody>
          {catalogue.map((compound) => <tr key={`${compound.formula}-${compound.name}`}><td><button onClick={() => { chooseCompound(compound); window.scrollTo({ top: 0, behavior: "smooth" }); }} type="button">{compound.name}</button></td><td>{compound.formula}</td><td>{compound.molecularWeight}</td><td>{compound.criticalTemperature}</td><td>{compound.criticalPressure}</td><td>{compound.acentricFactor}</td><td>{compound.antoineA}</td><td>{compound.antoineB}</td><td>{compound.antoineC}</td><td>{compound.antoineMin}–{compound.antoineMax}</td><td>{compound.formation?.phase ?? "—"}</td><td>{compound.formation?.enthalpy ?? "—"}</td><td>{compound.formation?.gibbsEnergy ?? "—"}</td></tr>)}
        </tbody></table></div>
        <p className="compound-table-note">Antoine form: ln(Pˢᵃᵗ [bar]) = A − B/(T [K] + C). Formation values use the gas-phase entry where listed because the Lee–Kesler calculation uses the vapour root. Values retain the precision printed in Koretsky.</p>
      </section>
    </section>
  );
}

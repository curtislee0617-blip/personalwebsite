...
export function RecipeCard({ entry, showBackLink = false }: { entry: RecipeCardEntry; showBackLink?: boolean }) {
  return (
    <details className="rounded-[1.5rem] border border-ink/10 bg-surface/55 p-4 transition hover:-translate-y-0.5 hover:border-ink/20 sm:p-5">
      <summary className="recipes-section-summary cursor-pointer list-none marker:hidden">
        <div className="relative overflow-hidden rounded-[1rem] border border-ink/10 bg-paper/70">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <span className="rounded-full border border-ink/10 bg-paper/80 px-3 py-1 text-xs font-semibold text-ink/50">{entry.status === "published" ? "Published" : "Coming later"}</span>
        </div>
        ...
        <p className="mt-5 text-sm font-semibold text-moss">Open recipe ↓</p>
      </summary>

...
export type NmrCouplingMarker = { id: string; shift: number; label: string };
...
type NmrSpectrumChartProps = {
  ...
  regions: NmrRegion[];
  couplingMarkers: NmrCouplingMarker[];
  solventPeak?: number;
  peaks: NmrPeak[];
...
export function NmrSpectrumChart({ points, xMinimum, xMaximum, axis, mode, regions, couplingMarkers, solventPeak, peaks, onRangeSelect, onPeakSelect }: NmrSpectrumChartProps) {
  ...
  function nearestPeak(target: number) {
    // Keep snapping local enough that adjacent multiplet lines remain
    // independently selectable. A broad full-spectrum radius previously made
    // both clicks in a ~7 Hz doublet/triplet collapse onto the tallest line.
    const radius = axis === "ppm"
      ? Math.max(span * 0.0015, 0.0025)
      : Math.max(span * 0.0005, 0.25);
  ...
  return <div className={`nmr-chart is-${mode}`}>
    <svg aria-label="Processed one-dimensional NMR spectrum" onPointerCancel={() => { setDragStart(null); setDragCurrent(null); }} onPointerDown={handleDown} onPointerLeave={() => { if (dragStart === null) setHover(null); }} onPointerMove={handleMove} onPointerUp={handleUp} role="img" viewBox={`0 0 ${width} ${height}`}>
      ...
      {couplingMarkers.map((marker) => {
        ...
        return <g className="nmr-coupling-marker" data-marker-id={marker.id} key={marker.id}>
          <line data-marker-part="line" x1={toX(shift)} x2={toX(shift)} y1={apexY} y2={toY(0)} />
          <circle cx={toX(shift)} cy={apexY} data-marker-part="dot" r="3.5" />
          <text textAnchor="middle" x={toX(shift)} y={Math.max(margin.top + 10, apexY - 8)}>{marker.label}</text>
        </g>;
      })}

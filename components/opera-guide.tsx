  ...
  return (
    <article className="cookbook-rail-card recipe-card scroll-mt-24 overflow-hidden rounded-[1.4rem] border border-ink/10 bg-surface/55 p-3" id={`opera-${recipe.slug}`}>
      ...
      {open && <div className="mt-3 grid gap-4 border-t border-ink/[0.07] p-4 sm:p-5">
        <RecipeImageViewer alt={`${recipe.title}, finished pastry`} className="relative mx-auto aspect-[4/3] w-full max-w-md overflow-hidden rounded-[1.05rem] bg-paper" src={recipe.image}><Image alt={`${recipe.title}, finished pastry`} className="object-cover" fill sizes="(max-width: 640px) 86vw, 28rem" src={recipe.image} /></RecipeImageViewer>

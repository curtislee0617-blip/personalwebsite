...
// The tiles render at roughly 62x103 CSS px, so the grid uses height-224 thumbs
// (photos/thumbs/, generated by scripts/generate-photo-grid-thumbs.mjs) instead
// of the full-size originals — ~640KB per cold visit instead of ~3.7MB. When the
// storage origin is known at build time the tiles go straight there, skipping
// the per-image 307 redirect that /photos/* otherwise costs.
const storageBase = process.env.NEXT_PUBLIC_SUPABASE_URL
  ? `${process.env.NEXT_PUBLIC_SUPABASE_URL.replace(/\/$/, "")}/storage/v1/object/public/site-media`
  : null;

function gridThumbUrl(photo: string) {
  const thumbPath = photo.replace(/^\/photos\//, "/photos/thumbs/");
  return storageBase ? `${storageBase}${thumbPath}` : thumbPath;
}
...
export function ScrollingPhotoBackground({
  ...
  return (
    <div className={rootClassName} aria-hidden="true">
      {/* No crossOrigin: the tiles are plain no-cors <img> fetches, and a CORS
          preconnect would open a connection they cannot reuse. */}
      {storageBase ? <link href={new URL(storageBase).origin} rel="preconnect" /> : null}
